		...
		<table width=600 align=center cellspacing=0 cellpadding=3>
			<form method=post action="sendto.jsp">
				<TR>
					...
					<Td><input name=from readonly
						value='<%=session.getValue("login")%>' style='width: 350px'></td>
				</tr>
				<TR>
					<TD align=right>To:</td>
					<td><input type=hidden value='admin_inbox' name=table><select
						name=to style='width: 350px'>
							<%
								try {		
									...
									ResultSet rs;
									rs = st.executeQuery("select UserEmail from admin");
									while (rs.next()) {
